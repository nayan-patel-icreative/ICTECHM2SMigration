...
class ProcessProductMigrationItemJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            if (is_string($previousFp) && $previousFp !== '' && is_string($fp) && $fp !== '' && hash_equals($previousFp, $fp)) {
                ...
                if ($mapping && is_string($mapping->shopify_gid) && $mapping->shopify_gid !== '') {
                    ...
                    if ($exists) {
                        ...
                        // Sync product visibility across specific market publications based on Magento visibilities
                        ...
                        $reportWriter->appendRow($run, [
                            ...
                            'product_sku' => (string) data_get($parent, 'sku', ''),
            ...
            // Translations are embedded in the already-fetched $parent data (no extra Magento API call)..
            ...
            // Upload Magento downloadable product files to Shopify Files CDN
            // and update product/variant metafields with the resulting CDN URLs.
            //
            // Magento private downloadable files (media.private=true, media.url="") are read
            ...
            try {
                ...
                if ($hasDigitalFiles) {
                    ...
                    if (count($variantDigitalFiles) > 0 && count($variantIdByMagentoId) > 0) {
                        ...
                        foreach ($variantDigitalFiles as $mageVariantId => $variantFiles) {
                            if ($mageVariantId === '' || count($variantFiles) === 0) {
                                continue;
                            }

                            // Only proceed for variants successfully migrated to Shopify
                            $shopifyVariantGid = $variantIdByMagentoId[$mageVariantId] ?? null;
                            if (!is_string($shopifyVariantGid) || $shopifyVariantGid === '') {
                                Log::warning('Variant digital files: no Shopify GID for Magento variant', [
                                    'run_id'        => $run->id,
                                    'shop'          => $shop->shop_domain,
                                    'magento_variant_id' => $mageVariantId,
                                ]);
                                continue;
                            }
