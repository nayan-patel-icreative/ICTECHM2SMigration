 ...
 *   3. If not → update the primary market currency to match Magento via marketCurrencySettingsUpdate.
...
class ShopifyPriceListSyncService
{
    ...
    /**
     * Ensure the primary market currency matches the Magento product currency,
...
GQL;
    ...
    private function ensurePrimaryMarketCurrency(Shop $shop, string $currencyCode): array
    {
        ...
        return $lock->block(30, function () use ($shop, $currencyCode, $cacheKey) {
            ...
            // Update the primary market currency to match Magento
            $mutation = <<<'GQL'
