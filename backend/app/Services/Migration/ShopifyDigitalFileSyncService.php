...
/**
 * Downloads Magento downloadable product files and uploads them to Shopify Files CDN,
 * returning the public CDN URLs.
 *
 * Magento downloadable files are defined via downloadable_product_links with link_type=file="" (empty).
 * The file lives in the Magento pub/media filesystem ({files_path}/media/xx/xx/xx/filename.ext).
 *
...
class ShopifyDigitalFileSyncService
{
     ...
     * @param string $magentoToken    Magento REST API Bearer token
     * @param string $magentoFilesPath  Absolute path to Magento media files dir
     *                                   e.g. "/var/www/html/magento2/pub/media"
    ...
    public function uploadDigitalFiles(
        ...
        foreach ($files as $file) {
            ...
            // If the Magento file entry has no file, there is no file — skip entirely.
            if (!$hasFile) {
                $results[] = [
                    'fileName'       => $displayName,
                    'shopifyFileUrl' => '',
                    'shopifyFileGid' => '',
                    'error'          => 'Magento media entity has no downloadable file',
                ];
                Log::warning('Digital file skipped — Magento no file', [
    ...
    private function tryAllStrategies(
        ...
        // Magento stores media files in {files_path}/media/xx/xx/xx/file.ext.
        ...
        // This strategy fetches from the Magento pub/media path but 404s in core.
    ...
    {
        $tmp = tempnam(sys_get_temp_dir(), 'mgdl_');
        if ($tmp === false) {
